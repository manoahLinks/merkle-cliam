...
const csvfile = path.join(__dirname, "airdrop_data.csv");
...
async function generateMerkleTree(csvFilePath: string): Promise<void> {
  ...
  await new Promise<void>((resolve, reject) => {
    fs.writeFile("../merkle_airdrop_tree.json", JSON.stringify(addressProofs), (err) => {
      if (err) {
        reject(err);
      } else {
        resolve();
  ...
  await new Promise<void>((resolve, reject) => {
    fs.writeFile("../address_erc1155_data.json", JSON.stringify(addressData), (err) => {
      if (err) {
        reject(err);
      } else {
        resolve();
